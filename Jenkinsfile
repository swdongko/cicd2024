          ...
          steps {
                withCredentials([
                  ...
                  string(credentialsId: 'PRISMA_SECRET_KEY', variable: 'PRISMA_SECRET_KEY')
                   ...
                   token=$(curl -sSLk -d "$json_auth_data" -H 'content-type: application/json' "$PCE_CONSOLE_URL/api/v1/authenticate" | python3 -c 'import sys, json; print(json.loads(sys.stdin)["token"])')
